...
def find_keys_in_code(path):
    ...
    # Filter out template literals (e.g. ${key})
    keys = {k for k in keys if not (k.startswith("${") and k.endswith("}"))}
...
def main():
    # Use current directory as base if running from within installer_bundle, 
    # otherwise assume script is in installer_bundle
    if os.path.exists("backup_app_v7_4.py"):
        base_dir = "."
    else:
